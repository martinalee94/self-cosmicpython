...
def test_can_allocate_if_available_greater_than_required():
    # GIVEN
    large_batch, small_order = make_batch_and_line(
        sku="SMALL-TABLE", batch_qty=20, line_qty=1
    )

    # WHEN
    result = large_batch.can_allocate(order_line=small_order)
...
def test_cannot_allocate_if_available_smaller_than_required():
    # GIVEN
    small_batch, large_order = make_batch_and_line(
        sku="SMALL-TABLE", batch_qty=2, line_qty=10
    )

    # WHEN
    result = small_batch.can_allocate(order_line=large_order)
...
def test_can_allocate_if_available_equal_to_required():
    # GIVEN
    batch, order = make_batch_and_line(sku="SMALL-TABLE", batch_qty=20, line_qty=20)

    # WHEN
    result = batch.can_allocate(order_line=order)
